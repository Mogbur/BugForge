...
def log_bug():
    ...
    base_filename = input(
        "Enter a base name for your bug report file (no extension): ").strip()
...
"""
    ...
    if save_csv:
        ...
        with open(csv_file, "a", newline='', encoding="utf-8") as file:
            ...
            if not file_exists:
                writer.writerow([
                    "Bug ID", "Timestamp", "Reporter", "Bug Title", "Severity",
                    "Platform/Device", "Expected Result", "Actual Result", "Status", "Attachments", "Steps to Reproduce"
                ])
            writer.writerow([
                bug_id, timestamp, reporter, title, severity, platform,
                expected, actual, status, attachments, " | ".join(steps_lines)
            ])
